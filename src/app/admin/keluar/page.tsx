  ...
  const load = useCallback(async (targetMonth: number, targetYear: number, signal: AbortSignal, requestSeq: number) => {
    ...
    try {
      const [atkData, rtData, obatData, grouped] = await Promise.all([
        getKeluar('atk', targetMonth, targetYear, { signal }),
        getKeluar('rt', targetMonth, targetYear, { signal }),
        getKeluar('obat', targetMonth, targetYear, { signal }),
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-gray-900">Persediaan Keluar</h1>
      ...
      <div className="bg-white rounded-xl shadow border border-gray-200">
        <div className="px-4 py-3 border-b flex items-center justify-between gap-3">
          ...
          />
          <button onClick={openAdd} className="shrink-0 flex items-center gap-1.5 bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 transition-colors">
            <Plus size={15} /> Tambah
          </button>
        </div>
